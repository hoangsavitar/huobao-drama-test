...
9) LANGUAGE LOCK: Output must be **100% English** (titles, script, intros, labels, names, dialogue, stage directions). Do not output Vietnamese or mixed-language text.
...
- **SCENE block:** `[SCENE: English line — time — mood]` then a separate line `*(English-only static environment description for background prompts; no characters or actions in this parenthetical.)*`
- **CHARACTER INTRO:** On **first appearance in this episode**, `[CHARACTER INTRO: Name]` then `*(150–300 words in English, highly detailed physical traits / age / styling / clothing as a visual blueprint; even minor characters should still receive at least ~80 words, never one-line placeholder intros.)*`
