...
3. **Important**: Scene descriptions must be a wide-angle "master background" that is clean and spacious, without any characters, people, or actions.
4. Prompt requirements:
   - Must use **English**, no Chinese characters
   - Detailed description of scene, time, atmosphere, style
   - Must have clear, empty floor/ground space in the center and foreground for characters to stand on
   - Explicitly forbid large blocking objects in the foreground (e.g., no close-up desks or tables blocking the lower half of the image)
...
2. For each character, extract:
   ...
   - outfits: An array of specific clothing sets the character wears in this script. Each outfit object must contain:
     - name: Name of the outfit (e.g., "Prison Uniform", "Prom Dress")
     - prompt: Detailed visual description of the outfit for AI image generation. Must describe a complete full-body outfit from head to toe (including footwear, bottoms, tops, outerwear), focusing on material, color, and fit.
3. Appearance must be detailed enough for AI image generation, including: gender, age, body type, facial features, hairstyle, etc. It must describe a simple, straight-on front-facing portrait in a strict A-Pose. The background MUST be pure white (no background, like a 3D game model character sheet). Base appearance should NOT include specific clothing if outfits are provided.
