...
func (s *ImageGenerationService) ProcessImageGeneration(imageGenID uint) {
	...
	isSceneImage := imageGen.ImageType == string(models.ImageTypeScene)
	if isSceneImage {
		prompt = ensureBackgroundOnlyPrompt(prompt)
		if imageGen.NegPrompt == nil || strings.TrimSpace(*imageGen.NegPrompt) == "" {
			sceneNegative := "people, person, human, man, woman, child, face, body, hands, crowd, silhouette, character, portrait"
			imageGen.NegPrompt = &sceneNegative
			opts = append(opts, image.WithNegativePrompt(sceneNegative))
		}
		if drama.Style != "" && drama.Style != "realistic" {
			prompt = fmt.Sprintf("%s visual style, %s", drama.Style, prompt)
		}
	}

	// If the drama has a style setting, add the style prompt
	if !isSceneImage && drama.Style != "" && drama.Style != "realistic" {
...
func (s *ImageGenerationService) completeImageGeneration(imageGenID uint, result *image.ImageResult) {
	...
	// If associated with a character, synchronously update the character's image_url and local_path (only if not an outfit)
	if imageGen.CharacterID != nil && imageGen.ImageType != string(models.ImageTypeOutfit) {
...
func (s *ImageGenerationService) getImageClient(provider string) (image.ImageClient, error) {
	...
	case "vertex":
		return image.NewVertexImageClient(model)
	case "gemini", "google":
