...
func (s *StoryboardService) UpdateStoryboard(storyboardID string, updates map[string]interface{}) error {
	...
	if val, ok := updates["narration"].(string); ok {
		updateData["narration"] = val
		sb.Narration = val
	}
	...
	if sb.Narration == "" && storyboard.Narration != nil {
		sb.Narration = *storyboard.Narration
	}
	...
	if charRaw, ok := updates["characters"]; ok {
		var charData []interface{}
		if cd, ok := charRaw.([]interface{}); ok {
			charData = cd
		}

		// Delete existing associations
		if err := s.db.Exec("DELETE FROM storyboard_characters WHERE storyboard_id = ?", storyboard.ID).Error; err != nil {
			return fmt.Errorf("failed to clear existing characters: %w", err)
		}
		...
		for _, item := range charData {
			...
			if id, ok := item.(float64); ok {
				...
				if oid, ok := m["outfit_id"].(float64); ok {
					val := uint(oid)
					outfitID = &val
				}
			}
			...
			if charID > 0 {
				if err := s.db.Exec("INSERT INTO storyboard_characters (storyboard_id, character_id, outfit_id) VALUES (?, ?, ?)",
					storyboard.ID, charID, outfitID).Error; err != nil {
					return fmt.Errorf("failed to assign character %d: %w", charID, err)
				}
			}
		}
	}
