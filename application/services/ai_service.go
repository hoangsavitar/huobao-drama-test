...
func (s *AIService) TestConnection(req *TestConnectionRequest) error {
	...
	switch req.Provider {
	case "vertex":
		s.log.Infow("Using Vertex AI client", "model", model)
		var err error
		client, err = ai.NewVertexClient(model)
		if err != nil {
			s.log.Errorw("Failed to create Vertex client", "error", err)
			return err
		}
	case "gemini", "google":
		// Gemini
...
func (s *AIService) GetAIClient(serviceType string) (ai.AIClient, error) {
	// For non-story text tasks, prefer Lite model when configured.
	if serviceType == "text" {
		liteClient, liteErr := s.GetAIClientForModel(serviceType, "gemini-3.1-flash-lite-preview")
		if liteErr == nil {
	...
	case "vertex":
		return ai.NewVertexClient(model)
	case "gemini", "google":
		return ai.NewGeminiClient(config.BaseURL, config.APIKey, model, config.Endpoint), nil
	default:
...
func (s *AIService) GetAIClientForModel(serviceType string, modelName string) (ai.AIClient, error) {
	...
	switch config.Provider {
	case "vertex":
		return ai.NewVertexClient(modelName)
	case "gemini", "google":
		return ai.NewGeminiClient(config.BaseURL, config.APIKey, modelName, config.Endpoint), nil
	default:
		// openai, chatfire and other providers all use OpenAI format
		return ai.NewOpenAIClient(config.BaseURL, config.APIKey, modelName, config.Endpoint), nil
	}
}
