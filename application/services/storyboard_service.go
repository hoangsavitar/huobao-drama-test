...
type Storyboard struct {
	...
	Narration        string         `json:"narration"`    // Voiceover narration when dialogue is empty
	...
	Characters       []uint         `json:"-"`            // List of involved character IDs
	CharacterOutfits map[uint]*uint `json:"-"`            // Map of character_id to outfit_id
	RawCharacters    []interface{}  `json:"characters"`   // Raw list of characters from AI
...
func (s *StoryboardService) GenerateStoryboard(episodeID string, model string) (string, error) {
	...
	// Get characters associated with this episode (from Step 2 extraction)
	var characters []models.Character
	if err := s.db.Preload("Outfits").
		Joins("INNER JOIN episode_characters ON episode_characters.character_id = characters.id").
		Where("episode_characters.episode_id = ?", episodeID).
		Order("characters.name ASC").
		Find(&characters).Error; err != nil {
		return "", fmt.Errorf("failed to get episode character list: %w", err)
	}

	// Fallback to drama-wide characters if episode-level characters are empty
	if len(characters) == 0 {
		if err := s.db.Preload("Outfits").Where("drama_id = ?", episode.DramaID).Order("name ASC").Find(&characters).Error; err != nil {
			return "", fmt.Errorf("failed to get character list: %w", err)
		}
	}

	// Build character list string (including ID, name, and available outfits)
	characterList := "No characters"
	if len(characters) > 0 {
		var charInfoList []string
		for _, char := range characters {
			outfitsStr := ""
			if len(char.Outfits) > 0 {
				var oList []string
				for _, o := range char.Outfits {
					oList = append(oList, fmt.Sprintf(`{"outfit_id": %d, "name": "%s"}`, o.ID, o.Name))
				}
				outfitsStr = fmt.Sprintf(`, "available_outfits": [%s]`, strings.Join(oList, ", "))
			}
			charInfoList = append(charInfoList, fmt.Sprintf(`{"id": %d, "name": "%s"%s}`, char.ID, char.Name, outfitsStr))
		}
		characterList = fmt.Sprintf("[%s]", strings.Join(charInfoList, ", "))
	}

	// Get extracted scene list for this episode (from Step 2 extraction)
	var scenes []models.Scene
	if err := s.db.Where("episode_id = ?", episodeID).Order("location ASC, time ASC").Find(&scenes).Error; err != nil {
		s.log.Warnw("Failed to get scenes for episode", "error", err)
	}

	// Fallback to drama-wide scenes if episode-level scenes are empty
	if len(scenes) == 0 {
		if err := s.db.Where("drama_id = ?", episode.DramaID).Order("location ASC, time ASC").Find(&scenes).Error; err != nil {
			s.log.Warnw("Failed to get scenes for drama", "error", err)
		}
	}
...
7. **Narration**: Keep narration separate from dialogue. If dialogue is non-empty, narration must be an empty string. If dialogue is empty, write third-person English short-drama recap narration in the style of Chinese/TikTok/Reels drama reviews, timed at about 2-3 words per second.
8. **Visual Result**: Immediate consequence of the action + visual details + atmosphere change
...
9. **Environmental Atmosphere**: Lighting quality + color tone + sound environment + overall mood
...
10. **BGM Prompt (bgm_prompt)**: Describe the mood, rhythm, and emotion of the background music for this shot (empty string if no special requirement)
   - Example: "Deep, tense strings, slow rhythm, creating an oppressive atmosphere"
11. **Sound Effect Description (sound_effect)**: Describe the key sound effects for this shot (empty string if no special effects)
    - Example: "Metallic clang, footsteps, sound of waves crashing"
12. **Audience Emotion**: [Emotion type] ([Intensity: ↑↑↑/↑↑/↑/→/↓] + [Resolution: Suspended/Released/Reversed])
...
- narration is separate from dialogue; never put narration into the dialogue field
- If dialogue is non-empty, narration MUST be ""
- If dialogue is empty, narration MUST be third-person English short-drama recap narration at about 2-3 words per second of shot duration

**Character and Scene Requirements**:
- The characters field must include all characters appearing in the shot. It must be an array of objects, where each object contains an "id" (the character ID) and an "outfit_id" (the specific outfit ID the character is wearing, or null if no specific outfit applies).
- Only extract characters that actually appear; use an empty array [] if no characters appear
...
- **Outfit IDs must strictly use the outfit_id field (numeric) from the character's available outfits in the list. Use null if they are in default clothing or no specific outfit matches.**
- Example: If Li Ming (id:159, wearing outfit_id: 5) and Wang Fang (id:160, base appearance) appear, the characters field should be: [{"id": 159, "outfit_id": 5}, {"id": 160, "outfit_id": null}]
...
func (s *StoryboardService) processStoryboardGeneration(taskID, episodeID, model, prompt string) {
	...
	if err := utils.SafeParseAIJSON(text, &storyboards); err == nil {
		// Parse raw characters
		for i := range storyboards {
			parseRawCharacters(&storyboards[i])
			normalizeStoryboardNarration(&storyboards[i])
		}
		...
		for i := range result.Storyboards {
			parseRawCharacters(&result.Storyboards[i])
			normalizeStoryboardNarration(&result.Storyboards[i])
		}
...
func (s *StoryboardService) generateVideoPrompt(sb Storyboard) string {
	...
	} else if sb.Narration != "" {
		parts = append(parts, fmt.Sprintf("Narration voiceover: %s", sb.Narration))
	}
	...
	return s.db.Transaction(func(tx *gorm.DB) error {
		...
		for _, sb := range storyboards {
			...
			description := fmt.Sprintf("【Shot Type】%s\n【Movement】%s\n【Action】%s\n【Dialogue】%s\n【Narration】%s\n【Result】%s\n【Emotion】%s",
				sb.ShotType, sb.Movement, sb.Action, sb.Dialogue, sb.Narration, sb.Result, sb.Emotion)
			...
			if len(sb.Characters) > 0 {
				var characters []models.Character
				if err := tx.Preload("Outfits").Where("id IN ?", sb.Characters).Find(&characters).Error; err != nil {
					...
					if err := tx.Model(&scene).Association("Characters").Append(characters); err != nil {
						...
						for _, char := range characters {
							var bestOutfitID *uint

							// 1. Try AI assigned outfit ID first
							if oid, ok := sb.CharacterOutfits[char.ID]; ok && oid != nil {
								bestOutfitID = oid
							} else if len(char.Outfits) > 0 {
								// 2. Fallback to basic keyword matching
								sbContext := strings.ToLower(fmt.Sprintf("%s %s %s %s", sb.Location, sb.Action, sb.Title, sb.Dialogue))
								for _, outfit := range char.Outfits {
									outfitName := strings.ToLower(outfit.Name)
									if outfitName != "" && strings.Contains(sbContext, outfitName) {
										id := outfit.ID
										bestOutfitID = &id
										break
									}
							...
							if bestOutfitID != nil {
								tx.Table("storyboard_characters").
									Where("storyboard_id = ? AND character_id = ?", scene.ID, char.ID).
									Update("outfit_id", bestOutfitID)
								s.log.Infow("Assigned outfit to character", "character_id", char.ID, "outfit_id", *bestOutfitID, "storyboard_id", scene.ID)
							}
						}
					}
				}
...
type CreateStoryboardRequest struct {
	...
	CharacterOutfits []struct {
		CharacterID uint  `json:"character_id"`
		OutfitID    *uint `json:"outfit_id"`
	} `json:"character_outfits"`
}
...
func (s *StoryboardService) CreateStoryboard(req *CreateStoryboardRequest) (*models.Storyboard, error) {
	...
	sb := Storyboard{
		...
		Narration:   getString(req.Narration),
	...
	// Associate characters
	if len(req.CharacterOutfits) > 0 {
		for _, co := range req.CharacterOutfits {
			if co.CharacterID > 0 {
				if err := s.db.Exec("INSERT INTO storyboard_characters (storyboard_id, character_id, outfit_id) VALUES (?, ?, ?)",
					modelSB.ID, co.CharacterID, co.OutfitID).Error; err != nil {
					s.log.Warnw("Failed to assign character outfit during creation", "error", err, "char_id", co.CharacterID)
				}
			}
		}
	} else if len(req.Characters) > 0 {
		var characters []models.Character
...
func normalizeStoryboardNarration(sb *Storyboard) {
	sb.Dialogue = strings.TrimSpace(sb.Dialogue)
	sb.Narration = strings.TrimSpace(sb.Narration)
	if sb.Dialogue != "" {
		sb.Narration = ""
	}
}

// parseRawCharacters parses characters from AI response which can be simple IDs or objects with outfits
func parseRawCharacters(sb *Storyboard) {
	sb.Characters = []uint{}
	sb.CharacterOutfits = make(map[uint]*uint)

	for _, raw := range sb.RawCharacters {
		switch v := raw.(type) {
		case float64:
			// Just an ID
			id := uint(v)
			sb.Characters = append(sb.Characters, id)
		case map[string]interface{}:
			// Object format: {"id": 159, "outfit_id": 5}
			if idVal, ok := v["id"].(float64); ok {
				id := uint(idVal)
				sb.Characters = append(sb.Characters, id)
				if outfitVal, ok := v["outfit_id"].(float64); ok {
					outfitID := uint(outfitVal)
					sb.CharacterOutfits[id] = &outfitID
				} else {
					sb.CharacterOutfits[id] = nil
				}
			}
		}
	}
