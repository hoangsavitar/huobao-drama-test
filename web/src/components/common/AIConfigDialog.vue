...
const providerConfigs: Record<AIServiceType, ProviderConfig[]> = {
  text: [
    ...
    },
    {
      ...
      models: ["gemini-3.1-flash-lite", "gemini-2.5-pro", "gemini-3-flash-preview"],
    },
  ],
  image: [
    {
...
const fullEndpointExample = computed(() => {
  ...
  if (serviceType === "text") {
    if (provider === "vertex") {
      endpoint = "Vertex AI SDK (no REST endpoint)";
...
const testConnection = async () => {
  ...
  // Vertex AI doesn't need base_url/api_key validation
  if (form.provider !== "vertex") {
...
const handleProviderChange = () => {
  ...
  // 根据厂商自动设置 Base URL
  if (form.provider === "vertex") {
    form.base_url = "vertex-ai";
    form.api_key = "ADC";
  } else if (form.provider === "gemini" || form.provider === "google") {
