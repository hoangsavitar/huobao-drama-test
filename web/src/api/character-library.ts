...
export const characterLibraryAPI = {
  ...
  // Outfits
  createOutfit(characterId: string | number, data: { name: string; prompt: string }) {
    return request.post(`/characters/${characterId}/outfits`, data)
  },
  updateOutfit(characterId: string | number, outfitId: string | number, data: { name?: string; prompt?: string; image_url?: string; local_path?: string }) {
    return request.put(`/characters/${characterId}/outfits/${outfitId}`, data)
  },
  deleteOutfit(characterId: string | number, outfitId: string | number) {
    return request.delete(`/characters/${characterId}/outfits/${outfitId}`)
  },
  generateOutfitImage(characterId: string | number, outfitId: string | number, data: { model?: string }) {
    return request.post(`/characters/${characterId}/outfits/${outfitId}/generate-image`, data)
  }
}
