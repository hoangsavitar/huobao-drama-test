...
type Drama struct {
	...
	NarrativeIdea *string        `gorm:"type:text" json:"narrative_idea,omitempty"`
...
type Character struct {
	...
	BaseImagePrompt *string        `gorm:"type:text" json:"base_image_prompt,omitempty"`
...
type CharacterOutfit struct {
	...
	ImageURL    *string        `gorm:"type:varchar(500)" json:"image_url"`
	LocalPath   *string        `gorm:"type:text" json:"local_path,omitempty"`
	Appearances string         `gorm:"type:varchar(500)" json:"appearances,omitempty"`
	CreatedAt   time.Time      `gorm:"not null;autoCreateTime" json:"created_at"`
	UpdatedAt   time.Time      `gorm:"not null;autoUpdateTime" json:"updated_at"`
	DeletedAt   gorm.DeletedAt `gorm:"index" json:"-"`
}

func (o *CharacterOutfit) TableName() string {
...
type Episode struct {
	...
	ParentNodeID    *string        `gorm:"size:64" json:"parent_node_id,omitempty"`
	Choices         datatypes.JSON `gorm:"type:json" json:"choices,omitempty"`
	StateSnapshot   datatypes.JSON `gorm:"type:json" json:"state_snapshot,omitempty"`
...
type StoryboardCharacter struct {
	StoryboardID uint  `gorm:"primaryKey"`
	CharacterID  uint  `gorm:"primaryKey"`
	OutfitID     *uint `json:"outfit_id"`
}

func (StoryboardCharacter) TableName() string {
	return "storyboard_characters"
}
