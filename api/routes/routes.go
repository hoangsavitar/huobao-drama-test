...
func SetupRouter(cfg *config.Config, db *gorm.DB, log *logger.Logger, localStorage interface{}) *gin.Engine {
	...
	dramaHandler := handlers2.NewDramaHandler(db, cfg, log, transferService, imageGenService)
	...
	api := r.Group("/api/v1")
	{
		...
		characters := api.Group("/characters")
		{
			...
			// Outfits
			outfits := characters.Group("/:id/outfits")
			{
				outfits.POST("", characterLibraryHandler.CreateOutfit)
				outfits.PUT("/:outfit_id", characterLibraryHandler.UpdateOutfit)
				outfits.DELETE("/:outfit_id", characterLibraryHandler.DeleteOutfit)
				outfits.POST("/:outfit_id/generate-image", characterLibraryHandler.GenerateOutfitImage)
			}
		}
