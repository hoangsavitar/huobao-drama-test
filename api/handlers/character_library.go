...
func (h *CharacterLibraryHandler) CreateOutfit(c *gin.Context) {
	characterID := c.Param("id")
	var req services2.CreateOutfitRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.BadRequest(c, err.Error())
		return
	}

	outfit, err := h.libraryService.CreateCharacterOutfit(characterID, &req)
	if err != nil {
		response.InternalError(c, err.Error())
		return
	}

	response.Created(c, outfit)
}

func (h *CharacterLibraryHandler) UpdateOutfit(c *gin.Context) {
	outfitID := c.Param("outfit_id")
	var req services2.UpdateOutfitRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.BadRequest(c, err.Error())
		return
	}

	if err := h.libraryService.UpdateCharacterOutfit(outfitID, &req); err != nil {
		response.InternalError(c, err.Error())
		return
	}

	response.Success(c, gin.H{"message": "Outfit updated successfully"})
}

func (h *CharacterLibraryHandler) DeleteOutfit(c *gin.Context) {
	outfitID := c.Param("outfit_id")

	if err := h.libraryService.DeleteCharacterOutfit(outfitID); err != nil {
		response.InternalError(c, err.Error())
		return
	}

	response.Success(c, gin.H{"message": "Outfit deleted successfully"})
}

func (h *CharacterLibraryHandler) GenerateOutfitImage(c *gin.Context) {
	outfitID := c.Param("outfit_id")
	
	var req struct {
		Model string `json:"model"`
	}
	c.ShouldBindJSON(&req) // Ignore error since model might be optional
	
	imageGen, err := h.libraryService.GenerateOutfitImage(outfitID, h.imageService, req.Model)
	if err != nil {
		response.InternalError(c, err.Error())
		return
	}
