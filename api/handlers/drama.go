...
func NewDramaHandler(db *gorm.DB, cfg *config.Config, log *logger.Logger, transferService *services.ResourceTransferService, imageGeneration *services.ImageGenerationService) *DramaHandler {
	ai := services.NewAIService(db, log)
	dramaService := services.NewDramaService(db, cfg, log, ai)
	dramaService.SetImageGenerationService(imageGeneration)
	return &DramaHandler{
		db:                db,
...
func (h *DramaHandler) GenerateNarrativeEpisodes(c *gin.Context) {
	...
	taskID, err := h.dramaService.GenerateNarrativeEpisodes(dramaID, req)
	if err != nil {
		msg := err.Error()
		...
		if msg == "invalid drama ID" || strings.HasPrefix(msg, "invalid agent step") {
	...
	response.Success(c, gin.H{
		"task_id": taskID,
		"message": "Generating narrative episodes in background",
	})
}
